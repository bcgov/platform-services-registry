export const up = async (db, client) => {
  ...
  if (oldPrivateCloudProject.length > 0) {
    await db.collection('PrivateCloudProject').rename('PrivateCloudProduct', { dropTarget: true });
  ...
  if (oldPrivateCloudRequestedProject.length > 0) {
    await db.collection('PrivateCloudRequestedProject').rename('PrivateCloudRequestData', { dropTarget: true });
  ...
  if (oldPublicCloudProject.length > 0) {
    await db.collection('PublicCloudProject').rename('PublicCloudProduct', { dropTarget: true });
  ...
  if (oldPublicCloudRequestedProject.length > 0) {
    await db.collection('PublicCloudRequestedProject').rename('PublicCloudRequestData', { dropTarget: true });
