...
import { Prisma, TaskType, RequestType, DecisionStatus } from '@/prisma/client';
...
async function decorate<T extends PublicCloudRequestSimple | PublicCloudRequestDetail>(
  ...
  const canCancel =
    doc.decisionStatus === DecisionStatus.PENDING &&
    (session.user.idirGuid === doc.createdBy?.idirGuid ||
      (session.permissions.cancelPublicCloudRequest && getDaysSince(doc.createdAt) > 30));
