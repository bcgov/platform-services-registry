...
type Handler<T = any> = (req: NextRequest, options?: { params: any }) => Promise<Response<T>>;
...
async function wrapHanlder<T>(handler: Handler<T>, req: NextRequest, options?: { params: any; data?: any }) {
  ...
  if (contentType.includes('application/json')) {
    try {
      ...
      res.json = async () => resData as T;
      isJSON = true;
    } catch {}
  } else {
    try {
      resData = await res.text();
      res.text = async () => resData as string;
    } catch {}
  }
...
export function createRoute(baseUrl: string) {
  return {
    ...
    delete: async function <T>(handler: Handler, url: string, paramData?: ParamData, headers?: any) {
      const { url: _url, pathParams } = populatePathParams(`${baseUrl}${url}`, paramData);
