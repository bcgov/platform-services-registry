...
import { archiveTeams, listTeams } from '@/services/backend/teams';
...
export default Page(({ session }) => {
  ...
  const handleArchiveSelected = async () => {
    ...
    try {
      setIsArchiving(true);
      await archiveTeams(selectedRowIds);
      setSelectedRowIds([]);
      await refetch();
      success({
        title: 'Teams archived',
        message: `${selectedRowIds.length} team${selectedRowIds.length === 1 ? '' : 's'} archived.`,
      });
    } catch (error) {
  ...
  return (
    <div className="pt-5 space-y-4">
        ...
        <div className="flex items-center justify-between gap-4 rounded-sm border border-slate-200 bg-slate-50 px-4 py-3">
          <div className="text-sm text-slate-700">
            {selectedRowIds.length > 0
              ? `${selectedRowIds.length} team${selectedRowIds.length === 1 ? '' : 's'} selected`
              : 'Select one or more teams to archive them in bulk.'}
          </div>
          <Button
            color="red"
      ...
      <DataTable
        ...
        getRowHref={(item) => `/teams/${item.id}`}
        selectableRows={session?.permissions.manageTeams}
        ...
        columns={[
          {
            ...
            width: '20rem',
            cellFormatter: (item) => (
              <Link href={`/teams/${item.id}`} className="block truncate" title={item.name || '(Unnamed team)'}>
