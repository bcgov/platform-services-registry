...
import { createTeamFromPublicCloudProduct, type TeamBootstrapResult } from './bootstrap-team-from-product';
...
async function main() {
  ...
  for (const product of products) {
    const result = await createTeamFromPublicCloudProduct(product);
    results.push(result);
  }
  ...
  logger.info('create-team-from-public-cloud-product completed', summary);
  console.log(
    `${summary.created} Teams created from ${summary.processed} Public Cloud Products. ${summary.alreadyExists} Teams already existed with links to products and were ignored. ${summary.failed} products failed.`,
  );
  console.log(JSON.stringify(summary, null, 2));
}

main().catch((error) => {
  logger.error('create-team-from-public-cloud-product failed', error);
