...
export default function ModifiedTeamContacts({
  ...
  const [formProjectOwner, formPrimaryTechnicalLead, formSecondaryTechnicalLead] = watch([
    'projectOwner',
  ...
  const handleUpdateContact = async (index: number) => {
    ...
    if (state.user) {
      ...
      setValue(['projectOwner', 'primaryTechnicalLead', 'secondaryTechnicalLead'][index], updatedUser, {
        shouldDirty: true,
      });
    }
  };
  ...
  const userAttributes = [
    ...
    },
    {
      ...
      content:
        'The DevOps specialist handles technical queries and platform updates. A Primary Technical Lead is required, while a Secondary is optional.',
    },
    {
  ...
  const filteredUserAttributes = userAttributes
    ...
    .map(({ role, user }, index) => (
      <Table.Tr key={index}>
        <Table.Td>
          {role}
          {index === 2 && <span className="italic font-bold"> (Optional)</span>}
        </Table.Td>
        <Table.Td>
          <Group
            gap="sm"
    ...
    <>
      <>
        ...
        <ul className="list-disc pl-5 ml-2">
          ...
          <li>
            <span className="font-semibold">Secondary Technical Lead (TL)</span>: {userAttributes[2].content}
          </li>
        </ul>
      </>
