...
def fetch_products_mark_completed_test(mongo_conn_id):
    """
    Fetches Approved Requests from MongoDB for Test Env, makes call to the Test Provisioner by every of them
    and if parameter completed = true and phase = Succeeded, makes a call to callback URL to mark Product complete.
    ...
    provisioner_url_test = os.environ["PROVISIONER_URL"][:23] + "-test" + os.environ["PROVISIONER_URL"][23:] + "test/"
    mark_provisioned_test_url = os.environ["MARK_PROVISIONED_URL"][:8] + \
        "test-" + os.environ["MARK_PROVISIONED_URL"][8:]
    ...
    try:
        ...
        for request in product_requests:
            ...
            if "lab" in cluster:
                ...
                provisioner_workflow_url = provisioner_url_test + workflow_name
                ...
                if request_phase != "Running" and 'workflows.argoproj.io/completed' in request_status_in_provisioner['metadata']['labels']:
                    ...
                    if request_phase == "Error":
                        ...
                        mark_provisioned_url = f"{mark_provisioned_test_url}{licence_plate}"
